...
def ready_response(vAR_input_model):
    w11,col11,col22,w22=st.columns((1,4,5,.5))
    cc2,cc1,cc3=st.columns((2,8,0.2))
    with col11:
        st.write('# ')
        st.write('# ')
        st.markdown("<p style='text-align: left; color: black; font-size:20px;'><span style='font-weight: bold'>Start Interaction with the Model</span></p>", unsafe_allow_html=True)
    with col22:
